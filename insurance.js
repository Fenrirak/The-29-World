...
async function render() {
    ...
    div.innerHTML = `
      <div class="flex-between">
        <div>
          ...
          <p class="muted-small">Covers: ${p.coverage || "—"}</p>
          <p><strong>${fmtMoney(p.price)}</strong> premium &middot; ${fmtMoney(p.excess)} excess ${p.stars ? `&middot; <span class="ticker-up">${stars(p.stars)}</span>` : ""}</p>
        </div>
        <div>
          ${IS_TEACHER
            ...
            : owned
              ...
              : `<button class="btn small gold" onclick="buyPlan('${p.id}')">${icon("shield", 13)} Buy cover</button>`}
        </div>
      </div>
      <div id="msg-${p.id}"></div>
    `;
  ...
  if (!IS_TEACHER) {
    ...
    mine.forEach(p => {
      ...
      row.innerHTML = `<div class="auto-details">${icon("shield", 14)} <strong>${p.name}</strong> &middot; ${fmtMoney(p.excess)} excess</div>`;
      box.appendChild(row);
    });
  }
}
...
async function addPlan(e) {
  ...
  const plan = {
    ...
    coverage: document.getElementById("pCoverage").value.trim(),
  ...
  ["pName","pPrice","pExcess","pCoverage","pDesc"].forEach(id => document.getElementById(id).value = "");
...
async function buyPlan(id) {
  ...
  document.getElementById("msg-" + id).innerHTML = res.ok
    ? `<div class="success-msg">You're covered!</div>`
